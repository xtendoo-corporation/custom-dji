# © 2004-2009 Tiny SPRL (<http://tiny.be>).
# © 2014-2017 Tecnativa - Pedro M. Baeza
# © 2016 ACSONE SA/NV (<http://acsone.eu>)
# License AGPL-3 - See http://www.gnu.org/licenses/agpl-3.0.html
#
# Fork mantenido por Xtendoo desde 2026-08-24: OCA descontinuó este módulo
# en purchase-workflow a partir de 16.0 (no existe en 17.0 ni posteriores).
# Renombrado con prefijo xtendoo_ para evitar colisión de nombre técnico
# con el módulo OCA original. Se mantiene la autoría/licencia original.
{
    "name": "Purchase order lines with discounts",
    "author": "Tiny, "
    "Acysos S.L., "
    "Tecnativa, "
    "ACSONE SA/NV,"
    "GRAP,"
    "Odoo Community Association (OCA),"
    "Xtendoo",
    "version": "19.0.1.0.2",
    "category": "Purchase Management",
    "website": "https://github.com/OCA/purchase-workflow",
    "depends": ["purchase_stock"],
    "data": [
        "views/xtendoo_purchase_discount_view.xml",
        "views/report_purchaseorder.xml",
        "views/product_supplierinfo_view.xml",
        "views/res_partner_view.xml",
    ],
    "license": "AGPL-3",
    "installable": True,
    "images": ["images/xtendoo_purchase_discount.png"],
}
